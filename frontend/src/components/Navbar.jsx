...
const Navbar = ({ darkMode, setDarkMode }) => {
    ...
    const handleMouseLeave = () => {
        timeoutRef.current = setTimeout(() => setActiveDropdown(null), 150) // small delay
    }
                ...
                <div className="flex justify-between items-center h-14 sm:h-16 md:h-20">
                    ...
                    >
                        ...
                        <div className="flex flex-col min-w-0 overflow-hidden">
                            ...
                            <div
                                className={`text-[10px] sm:text-xs font-medium mt-0.5 sm:mt-1 ${darkMode ? "text-white" : "text-slate-900"
                        ...
                        {navLinks.map((link) => (
                            ...
                            >
                                {link.hasDropdown ? (
                                    <>
                                        <button
                                            className={`flex items-center space-x-1 transition-colors duration-300 text-sm font-medium whitespace-nowrap ${darkMode
                                        ...
                                        <div
                                            className={`absolute top-full left-0 mt-2 w-56 backdrop-blur-lg rounded-lg shadow-xl transition-all duration-300 z-50 ${darkMode
                                                ? "bg-[#1e293b]/95 border border-gray-700"
                                    ...
                                    <Link
                                        to={link.href}
                                        className={`transition-colors duration-300 text-sm font-medium whitespace-nowrap ${darkMode
                                            ? "text-gray-300 hover:text-white"
                                            : "text-slate-700 hover:text-slate-900"
                                            }`}
                                    >
                                    ...
                                    >
                                        {link.items?.map((item) => (
                                            <Link
                                                ...
                                                className={`block px-6 sm:px-8 py-2.5 text-sm transition-colors ${darkMode
                                                    ? "text-gray-400 hover:text-[#5eaaa8]"
                                                    : "text-slate-600 hover:text-[#5eaaa8]"
                                                    }`}
                                            >
                                                {item.name}
                                            </Link>
